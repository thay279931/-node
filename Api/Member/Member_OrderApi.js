...
router.use('/', async (req, res) => {
    ...
    //有登入才叫資料
        ...
        let sql = "SELECT o.`SID`,o.`member_sid`,o.`shop_sid`,o.`shop_order_status`,o.`order_finish`,o.`order_time`,o.`recept_time`,o.`complete_time`,o.`order_total`,o.`memo`,o.`coupon_sid`,o.`sale`,o.`review`,o.`chat_sid`,o.`canceled_reason`,o.`who_canceled`,s.`name`,cc.`coupon_name` FROM `orders` o LEFT JOIN `shop` s ON o.`shop_sid` = s.`sid` LEFT JOIN `coupon` c ON o.`coupon_sid`=c.`sid` LEFT JOIN `coupon_content` cc ON c.`coupon_sid` = cc.`sid` WHERE o.member_sid = ?";
